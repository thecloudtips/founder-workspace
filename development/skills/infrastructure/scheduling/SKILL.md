...
When a user runs e.g. `/founder-os:briefing:daily --schedule "weekdays 7:30am"`:

1. Parse the schedule expression (NL or cron)
2. Generate workflow YAML from template at `templates/workflow-template.yaml`
3. Write to `workflows/auto-[command-name].yaml`
4. Read P27's scheduling skill at `skills/workflow/workflow-scheduling/SKILL.md`
...
Use P27's existing NL-to-cron conversion table (from `skills/workflow/workflow-scheduling/SKILL.md`):
...
|---------|-------------------|
| `/founder-os:briefing:daily` | Weekdays 7:30am |
| `/founder-os:morning:sync` | Weekdays 8:00am |
| `/founder-os:review:weekly` (`review`) | Fridays 5:00pm |
| `/founder-os:crm:sync-email` | Weekdays 9:00am, 2:00pm |
| `/founder-os:client:health-scan` (`health-scan`) | Weekdays 9:30am |
| `/founder-os:followup:check` | Weekdays 10:00am |
| `/founder-os:drive:search` (`drive-search`) | Daily 6:00am |
